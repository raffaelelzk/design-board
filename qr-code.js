/*
 * Local QR generator for Creative Toolbox.
...
module.exports = QRCode;
  ...
  var QRErrorCorrectLevel = localRequire("QRErrorCorrectLevel");

  function render(canvas, text, options) {
    options = options || {};
    var value = String(text || "");
    if (!value) throw new Error("二维码内容为空");
    // Version 40-L holds roughly 2.9k bytes. Keep margin for stable scanning.
    if (value.length > (options.maxLength || 1800)) {
      throw new Error("内容超过单个二维码容量");
    }
    ...
    var quiet = options.quietZone == null ? 4 : options.quietZone;
    var target = options.size || 320;
    var scale = Math.max(2, Math.floor(target / (count + quiet * 2)));
    var size = (count + quiet * 2) * scale;
    var ctx = canvas.getContext("2d");
    canvas.width = size;
    canvas.height = size;
    canvas.style.width = Math.min(target, size) + "px";
    canvas.style.height = Math.min(target, size) + "px";

    ctx.fillStyle = options.background || "#ffffff";
    ctx.fillRect(0, 0, size, size);
    ctx.fillStyle = options.foreground || "#111827";
    ...
    return { moduleCount: count, size: size, scale: scale };
  }

  global.CreativeQR = { render: render, maxTextLength: 1800 };
})(window);
